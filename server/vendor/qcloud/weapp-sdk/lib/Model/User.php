...
class User
{
    public static function storeUserInfo ($userinfo, $skey, $session_key) {
        ...
        if ($res === NULL) {
            DB::insert('cSessionInfo', compact('uuid', 'skey', 'create_time', 'last_visit_time', 'open_id', 'session_key', 'user_info','nick_name'));
        } else {
            DB::update(
                'cSessionInfo',
                compact('uuid', 'skey', 'last_visit_time', 'session_key', 'user_info','nick_name'),
                compact('open_id')
            );
        }
    }
